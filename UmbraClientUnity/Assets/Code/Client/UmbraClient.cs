...
namespace ClientLib
{
	public class UmbraClient
	{
		...
		public void SendAuth(string authkey, int x, int y) { Send("auth", authkey, x, y); }
		public void SendSetPosition(string room, int x, int y) { Send("set-position", room, x, y); }
		...
		public delegate void PositionUpdateHandler(string cid, string room, int x, int y);
		public delegate void SomeoneJoinedHandler(string cid, string displayName, int x, int y);
		...
		private void ParseAndDoleOut(string feed)
		{
			...
			if (cmd == "position")
			{
				PositionUpdate(cid, p[0], int.Parse(p[1]), int.Parse(p[2]));
			}
			else if (cmd == "say")
			{
			...
			{
				SomeoneJoined(cid, p[0], int.Parse(p[1]), int.Parse(p[2]));
			}
			else if (cmd == "drop")
			{
				SomeoneDropped(cid);
